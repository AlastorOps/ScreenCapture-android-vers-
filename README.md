...
- **Phase 3: screen casting** — drives a vendored [scrcpy](https://github.com/Genymobile/scrcpy)
  ...
  `androidlink/streaming/transport.py`'s module docstring for details.
...
- **Phase 5: Android audio → PC** — plays the device's audio (Opus by default) through
  ...
  the same hardware-unverified caveat as Phases 3-4.
...
- **Phase 8: PC-side recording** — records the live cast to an MP4 file entirely on the PC
  ...
  (deferred to Phase 9's full Settings page). **Video-only for this pass** — audio isn't
  muxed into recordings yet, since Android audio lives entirely inside its own worker
  thread (streaming/transport.py) with no tap point exposed to the recorder; that's a real
  gap, documented here rather than silently dropped. The encoder-selection/fallback logic
  and the full record-then-decode-back round trip are tested against a real written MP4,
  not mocked.

Polish (Phase 9) is not implemented yet.
